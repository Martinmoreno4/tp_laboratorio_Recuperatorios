...
int controller_addEmployee(LinkedList* pArrayListEmployee)
{
    ...
    Employee* auxEmployee=newEmployee();
    int todoOk=0;
    int id=1001;
    int *indice;
    ...
    if(path != NULL && pArrayListEmployee != NULL)
    {
        ...
        if(fp!=NULL)
        {
            for(int i=0; i<len; i++)
            {
                ...
                if(auxEmployee!=NULL)
                {
                    fwrite(auxEmployee, sizeof(auxEmployee), len, fp);
                    //fprintf(fp,"%d, %s, %d, %d", auxEmployee->id, auxEmployee->nombre,auxEmployee->horasTrabajadas,auxEmployee->sueldo);
                    error =1;
                }
            }
        }
...
/*int obtenerIdMasAlto(LinkedList* pArrayListEmployee)
{
    int len = ll_len(pArrayListEmployee);
    int idMayor=-1;
    int idMayorFlag=1;
    if(pArrayListEmployee != NULL)
    {
        Employee* auxEmployee;
            for(int i=0; i<len; i++)
            {
                ...
                else if(auxEmployee->id >idMayor)
                {
                    idMayor=auxEmployee->id;
                }
